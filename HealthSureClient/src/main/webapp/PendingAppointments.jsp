...
<f:view>
    <h:form>
        <h2><h:outputText value="Pending Appointments" /></h2>

        <h:commandButton value="Load Pending Appointments" action="#{appointmentController.loadAllAppointments}" />
        ...
        <h:dataTable value="#{appointmentController.allAppointments}" var="appt"
            ...
            <h:column>
                ...
                <h:commandButton value="Book"
                                 action="#{appointmentController.approve(appt.appointmentId)}"
                                 rendered="#{appt.status eq 'PENDING'}" />
            </h:column>

            <h:column>
                <f:facet name="header"><h:outputText value="Cancel" /></f:facet>
                <h:commandButton value="Cancel"
                                 action="#{appointmentController.cancel(appt.appointmentId)}"
                                 rendered="#{appt.status eq 'PENDING'}" />
            </h:column>
